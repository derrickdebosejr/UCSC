...
void window::putObj(object obj){
   window::objects.push_back(obj);
}
...
void window::move_selected_object(int directions){
   GLfloat delta = interpreter::get_moveby();
   DEBUGF ('g', "delta " << delta );
   if(window::selected_obj >= window::objects.size()){
      return;
   }
   if(directions == 0){
      window::objects[window::selected_obj].move(0.0-delta,0.0);
   }else if(directions == 1){
      window::objects[window::selected_obj].move(0.0,0.0-delta);
   }else if(directions == 2){  
      window::objects[window::selected_obj].move(0.0,delta);
   }else if(directions == 3){
      window::objects[window::selected_obj].move(delta,0.0);
   }
   window::display();
   glutPostRedisplay();
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      ...
      case 'H': case 'h':
         ...
         window::move_selected_object(0);
         break;
      case 'J': case 'j':
         //move_selected_object (
         window::move_selected_object(1);
         break;
      case 'K': case 'k':
         //move_selected_object (
         window::move_selected_object(2);
         break;
      case 'L': case 'l':
         //move_selected_object (
         window::move_selected_object(3);
         break;
      ...
      case '5': case '6': case '7': case '8': case '9':
         ...
         DEBUGF ('g', "select_object " << window::selected_obj);
         break;
      default:
